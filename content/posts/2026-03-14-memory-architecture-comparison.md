...
**TL;DR** — I compared [crib](https://github.com/bioneural/crib), my memory system for [Prophet](https://github.com/bioneural/prophet), against [Google's always-on memory agent](https://github.com/GoogleCloudPlatform/generative-ai/tree/main/gemini/agents/always-on-memory-agent), a daemon that loads stored memories into the model's context and lets the model decide relevance. Crib searches three channels — full-text, vector, triples — then fuses and reranks. Google's daemon hands up to 50 memories to the model at query time. It introduces three patterns crib lacks: background consolidation, bidirectional connection graphs, multimodal ingestion. Neither solves cold-start context injection — the most consequential gap for Prophet.
