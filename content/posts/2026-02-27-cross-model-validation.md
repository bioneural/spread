...
**TL;DR** — Prophet's eval suite (109 cases, 6 suites) has only ever run against gemma3:1b. Running it against gemma3:4b: five of six suites produce identical scores. Only entity-triple retrieval improves (F1 0.895 to 0.976). Memory extraction stays at F1 0.0 on both. Most of Prophet's retrieval quality comes from infrastructure — full-text search, entity graphs, preference injection — not model quality. A larger model helps at the margin but does not move the dominant suites.
