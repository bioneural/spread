...
**TL;DR** — Semantic similarity retrieval cannot serve dispositional knowledge. A preference like "simplicity matters more than performance" should inform every decision, not just queries that mention simplicity. The fix: a new entry type (`preference`) with a SQL query that always surfaces active preferences regardless of topic. F1 = 0.971 across 21 test cases. The one failure is a reranker discrimination limit, not an injection failure.
