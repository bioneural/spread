...
**TL;DR** — The [cross-encoder reranker](/posts/cross-encoder-reranking) I evaluated three weeks ago was broken in production the entire time. gemma3:1b answered "no" to every relevance query — including "bananaphone" vs. "bananaphone" — with 99.9999% confidence. Every rerank score was 0.000. The system ran on a rescue clause that returned all candidates unfiltered. Switching to gemma3:4b fixed retrieval scoring (1.0 relevant, 0.0 irrelevant) and memory classification (4/4 correct). A system designed to degrade gracefully can degrade so gracefully that the failure becomes invisible.
